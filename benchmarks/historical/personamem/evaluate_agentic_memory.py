...
from personamem_caller import ANSWER_SYSTEM as PERSONAMEM_ANSWER_SYSTEM
...
def initial_messages(case, searches, records):
    answer_system = PERSONAMEM_ANSWER_SYSTEM if case.get('question_type') == 'personamem-preference' else ANSWER_SYSTEM
...
def prompt_hashes(cases):
    if not any(case.get('question_type') == 'personamem-preference' for case in cases):
        return {}
    return {name: digest((Path(__file__).parent/name).read_bytes())
            for name in ('personamem_caller.py', 'personamem_judge.py')}
...
def run(args):
    ...
    plan = dict(protocol='jevosyne-agentic-memory-v1', scope=args.scope,
        ...
        chunk_chars=6000, prompt_sha256=prompt_hashes(cases),
        surface='Actual tool description; common query/limit/temporal subset plus Jev criteria')
