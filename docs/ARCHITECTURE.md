...
The request pool bounds concurrency and queued work. All eligible records still participate, so reducing top-k does not eliminate corpus-scanning cost. Independent questions carry only their own memory span and criterion. Bounded native batches share an empty state and reuse pooled HTTPS connections. An exact per-decision cache survives batch repacking; a changed criterion or span requires a new decision. Parallel duplicate checks preserve the actual ordered comparator set, with recomputation when speculative selections differ. A failed decision operation causes an explicit failure rather than silently admitting a partial corpus result.
