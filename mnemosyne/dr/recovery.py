...
def create_backup(db_path: Path = None, backup_dir: Path = None) -> Dict:
    ...
    # Back up SQLite pages, preserving legacy virtual tables without loading
    # their extensions. SQL dumps cannot represent an unloaded vec0 table.
    with tempfile.TemporaryDirectory() as directory:
        ...
        try:
            ...
            src.close()
            dst.close()
        with snapshot.open("rb") as source, gzip.open(backup_path, "wb") as target:
...
def restore_backup(backup_path: Path, db_path: Path = None) -> Dict:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if payload.startswith(b"SQLite format 3\x00"):
            ...
            mem_db = sqlite3.connect(str(snapshot))
        else:
            # Historical Mnemosyne SQL backups remain readable when they
            # contain ordinary tables and FTS. Keep the original untouched.
            mem_db = sqlite3.connect(":memory:")
            mem_db.executescript(payload.decode("utf-8"))
