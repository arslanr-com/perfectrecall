...
class HindsightImporter(BaseImporter):
    ...
    def transform(self, raw_data: List[Dict]) -> List[Dict]:
        ...
        for item in raw_data:
            ...
            metadata["import_quality_score"] = self._quality_score(
    ...
    def _stable_id(self, item: Dict) -> str:
        raw = (
            item.get("id")
            or item.get("uuid")
            or self._content_for(item)
            or json.dumps(item, sort_keys=True, default=str)
        )
        digest = hashlib.sha256(f"{self.namespace}:{raw}".encode("utf-8")).hexdigest()[
            :24
        ]
        return f"hs_{digest}"
    ...
    def _importance_for(item: Dict) -> float:
        ...
        try:
            ...
            proof_bonus = 0.0
        base = {"world": 0.75, "experience": 0.65, "observation": 0.55}.get(
            fact_type, 0.5
        )
    ...
    def _quality_flags(content: str) -> List[str]:
        ...
        if re.search(
            "\\bConversation between\\b|\\bvia Telegram DM\\b|\\bon Telegram DM platform\\b",
            text,
            re.I,
        ):
            flags.append("generic_conversation_metadata")
        if re.search(
            "Review the conversation above|consider saving.*memory|Has the user revealed|Focus on:",
            text,
            re.I | re.S,
        ):
            flags.append("meta_memory_prompt")
        if re.search("\\b[A-Za-z0-9+/]{90,}={0,2}\\b", text) or "aaak" in lower:
            flags.append("dense_or_encoded_blob")
        if lower in {
            "no new narrow skills.",
            "- no new narrow skills.",
            "soucrce: convo",
            "source: convo",
        }:
    ...
    def run(
        ...
        result = ImporterResult(
            provider=self.provider_name, started_at=datetime.now().isoformat()
        )
        try:
            ...
            for mem in memories:
                try:
                    ...
                    inserted = self._insert_episodic(
                        conn, mem, generate_embeddings=self.generate_embeddings
                    )
                    if inserted:
                        result.imported += 1
            ...
            if result.imported > 0:
                logger.info(
                    "MEMORIA backfill: extracting facts from %d imported memories",
                    result.imported,
                )
                try:
                    beam = mnemosyne.beam
                    ...
                    logger.warning(
                        "MEMORIA backfill: failed (%s) — structured router will see 0 facts for imported memories",
                        e,
                    )
        except Exception as e:
    ...
    @staticmethod
    def _insert_episodic(conn, mem: Dict, generate_embeddings: bool = False) -> bool:
        metadata_json = json.dumps(
            mem.get("metadata", {}), ensure_ascii=False, sort_keys=True, default=str
        )
        cur = conn.execute(
        ...
        if inserted and generate_embeddings:
            HindsightImporter._backfill_import_embedding(
                conn, cur.lastrowid, mem["content"]
            )
        return inserted
...
def import_from_hindsight(
    ...
    return importer.run(
        mnemosyne, dry_run=dry_run, session_id=session_id, channel_id=channel_id
    )
