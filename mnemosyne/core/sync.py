...
def _event_sort_key(event: "SyncEvent") -> Tuple[datetime, str, str]:
    ...
    return (timestamp, event.device_id or "", event.event_id or "")
...
class SyncEncryption:
    ...
    def derive_key(
        ...
        try:
            ...
            kdf = _PBKDF2(
                algorithm=_hashes.SHA256(), length=32, salt=salt, iterations=600000
            )
            key = kdf.derive(passphrase.encode("utf-8"))
            return (key, salt)
        except ImportError:
            raise ImportError(
                "SyncEncryption requires either 'cryptography>=41.0' or 'argon2-cffi' for key derivation. Install with: pip install perfectrecall[sync]"
            )
    ...
    def encrypt_payload(payload: dict, key: bytes) -> str:
        ...
        try:
            ...
            raise ImportError(
                "SyncEncryption.encrypt_payload requires 'cryptography>=41.0' or 'PyNaCl>=1.5'. Install with: pip install perfectrecall[sync]"
            )
    ...
    def decrypt_payload(encrypted: str, key: bytes) -> dict:
        ...
        try:
            ...
            raise ImportError(
                "SyncEncryption.decrypt_payload requires 'cryptography>=41.0' or 'PyNaCl>=1.5'. Install with: pip install perfectrecall[sync]"
            )

    @classmethod
    def from_config(
        cls, key_source: Optional[str] = None, **kwargs
    ) -> Optional["SyncEncryption"]:
            ...
            if os.path.isfile(key_source):
                ...
                try:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "key_source is neither a file path nor a valid base64-encoded key"
                        )
...
class ConflictResolution:
    ...
    def resolve_with_chain(
        events: List[SyncEvent], parent_map: Optional[Dict[str, List[str]]] = None
    ) -> SyncEvent:
        ...
        **How a Hermes plugin would consume this**:
        ...
        for group in conflict_groups:
            ...
            best_idx = max(
                ...
                key=lambda i: (candidates[i]["importance"], candidates[i]["timestamp"]),
            )
...
class SyncEngine:
    ...
    def __init__(
        ...
        if self.require_encryption and encryption is None and (not self.relay_mode):
            raise ValueError(
                "encryption is required but no encryption key is configured"
            )
        ...
        if self.surface_only:
            ...
            if foreign_count:
                raise ValueError(
                    "surface-only sync requires a dedicated single-surface event log"
                )
    ...
    def _init_surface_namespace(self) -> None:
        ...
        columns = {
            row[1]
            for row in self.conn.execute("PRAGMA table_info(working_memory)").fetchall()
        }
    ...
    def _migrate_legacy_encrypted_payloads(self) -> None:
        ...
        try:
            for row in rows:
                ...
                event.payload = _ENCRYPTED_WIRE_PREFIX + self.encryption.encrypt(
                    envelope
                )
    ...
    def _init_events_table(self) -> None:
        ...
        event_columns = {
            row[1]
            for row in cursor.execute("PRAGMA table_info(memory_events)").fetchall()
        }
        ...
        if "apply_state" not in event_columns:
            cursor.execute(
                "ALTER TABLE memory_events ADD COLUMN apply_state TEXT NOT NULL DEFAULT 'applied'"
            )
    ...
    def _compute_event_hash(self, event: SyncEvent) -> str:
        ...
        raw = f"{event.memory_id}|{event.operation}|{event.timestamp}|{event.device_id}|{event.payload or ''}|{event.parent_event_ids}|{event.importance}"
    ...
    def log_event(
        ...
        if commit:
            self.conn.commit()
        logger.debug("Logged sync event %s: %s %s", event_id, operation, memory_id)
        return event
    ...
    def discover_local_mutations(self) -> Dict[str, Any]:
        ...
        try:
            if self.conn.in_transaction:
                raise RuntimeError(
                    "mutation discovery requires a clean SQLite transaction"
                )
    ...
    def _discover_local_mutations(self) -> Dict[str, Any]:
        ...
        stats: Dict[str, Any] = {"created": 0, "updated": 0, "deleted": 0, "events": []}
        ...
        for row in applied_rows:
            ...
            if previous_event is None or _event_sort_key(candidate) > _event_sort_key(
                previous_event
            ):
        ...
        for memory_id, payload in current.items():
            ...
            if latest is not None and latest.operation != "DELETE":
                ...
                if (
                    ...
                    and self._payload_fingerprint(latest_payload) == fingerprint
                ):
                    self._state_set(
                        memory_id,
                        fingerprint,
            ...
            self._state_set(
                memory_id, fingerprint, operation, event.event_id, event.timestamp
            )
        ...
        for memory_id, previous in state.items():
            ...
            if latest is not None and latest.operation == "DELETE":
                self._state_set(
                    memory_id, "", "DELETE", latest.event_id, latest.timestamp
                )
                continue
    ...
    def _decode_payload(self, event: SyncEvent) -> Optional[Dict[str, Any]]:
        ...
        if encrypted:
            ...
            if not isinstance(envelope_meta, dict) or not isinstance(
                envelope_payload, dict
            ):
                if self.require_encryption:
                    raise ValueError(
                        "encrypted payload has no authenticated sync envelope"
                    )
                return decoded
            expected_meta = {
                "version": 1,
        ...
        if self.require_encryption:
            raise ValueError(
                "plaintext payload rejected because encryption is required"
            )
    ...
    def _latest_event(
        ...
        rows = self.conn.execute(
            f"SELECT * FROM memory_events WHERE {' AND '.join(clauses)}", params
        ).fetchall()
        events = [SyncEvent.from_row(dict(row)) for row in rows]
    ...
    def _apply_memory_event(
        ...
        if event.operation == "DELETE":
            ...
            self._state_set(
                event.memory_id, "", "DELETE", event.event_id, event.timestamp
            )
            return
        ...
        if exists:
            try:
                ...
                pass
            present = {key: value for key, value in values.items() if key in available}
    ...
    def push_changes(self, events: List[dict]) -> Dict[str, Any]:
        ...
        for raw in events:
            try:
                ...
                if (
                    ...
                    and all(
                        (char in "0123456789abcdefABCDEF" for char in provided_hash)
                    )
                ...
                if existing_state is not None and existing_state != "pending":
                    ...
                    if stored_event.to_dict() != event.to_dict():
                        raise ValueError(
                            "duplicate event_id does not match stored event"
                        )
                    stats["duplicates"] += 1
                ...
                if existing_state == "pending":
                    ...
                    if stored_event.to_dict() != event.to_dict():
                        raise ValueError(
                            "pending retry does not match the stored event"
                        )
                    event = stored_event
        ...
        for index, (event, retry_pending) in enumerate(incoming):
            ...
            try:
                ...
                if latest is not None and _event_sort_key(latest) >= _event_sort_key(
                    event
                ):
                    if retry_pending:
        ...
        if mode in ("push", "bidirectional"):
            ...
            while True:
                ...
                if len(acknowledged) < len(batch):
                    result["errors"].append(
                        "remote only partially acknowledged a push batch"
                    )
                    break
            ...
            while True:
                ...
                if events:
                    ...
                    if int(applied.get("errors", 0) or 0) > 0:
                        result["errors"].append(
                            "pull page failed validation; cursor not advanced"
                        )
                        break
                ...
                if not events:
                    result["errors"].append(
                        "remote reported has_more without returning events"
                    )
                    break
    ...
    def get_status(
        self, remote_url: Optional[str] = None, api_key: Optional[str] = None
    ) -> Dict[str, Any]:
