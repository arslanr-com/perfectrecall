...
class PersonaExtractor:
    ...
    def extract_candidates(
        ...
        from . import jev
        if jev.enabled():
            from .jev_recall import persona_candidates
            return persona_candidates(self, session_id, limit)
    ...
    def deduplicate_by_topic(
        ...
        from . import jev
        if jev.enabled():
            return jev.deduplicate(sorted(candidates, key=lambda x: -x["importance"]))
...
def render_persona_markdown(
    ...
    if approx_tokens > token_cap:
        ...
        for topic, _ in topic_pri:
            ...
            section_text = "\n".join(section)
            candidate_words = current_words + len(section_text.split())
            if int(candidate_words * 1.3) > token_cap:
                continue
