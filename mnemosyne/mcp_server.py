"""
PerfectRecall MCP Server -- stdio, SSE and Streamable HTTP transports.
...
async def _run_stdio() -> None:
    ...
    if not _MCP_AVAILABLE:
        raise RuntimeError("MCP not installed. Run: pip install perfectrecall[mcp]")
        ...
        if multi_tokens is not None:
            ...
            if AuthenticatedUser is None or AccessToken is None:
                raise RuntimeError(
                    ...
                    "(AuthenticatedUser/AccessToken); install perfectrecall[mcp]."
                )
...
def run_mcp_server(
    ...
    """
    Run the PerfectRecall MCP server.
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    parser = argparse.ArgumentParser(description="PerfectRecall MCP Server")
    parser.add_argument(
        "--transport",
