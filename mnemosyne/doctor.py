...
class ReferenceContractRegistry:
    ...
    def inspect(self) -> AdapterResult:
        ...
        metrics["vec_working"] = {"status": "legacy_unused"}
...
class VectorCoverageAdapter:
    """Compatibility diagnostic: legacy vectors do not participate in recall."""
    def __init__(self, conn, scan_limit=DEFAULT_SCAN_LIMIT):
        self.conn = conn
    def inspect(self):
        return AdapterResult(metrics={"status": "not_applicable", "backend": "jev"}, findings=[], repair_candidates=[])
