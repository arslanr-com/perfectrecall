...
#   1. MnemosyneMemoryProvider.prefetch() renders a `## PerfectRecall Context`
#      block.
...
def _prefetch_topic_signal(row: Dict[str, Any]) -> float:
    ...
    signal = max(
        float(row.get("jev_relevance") or 0.0),
...
RECALL_SCHEMA = {'name': 'mnemosyne_recall', 'description': "Search PerfectRecall memory. Jev evaluates every eligible memory and returns original evidence records. Supply query and 1-3 evidence_questions. Each criterion must be a SHORT, self-contained yes/no question about ONE observable fact or topic; aim for 20 words or fewer. Use ordinary concrete words. Do not pack lists of attributes, eligibility conditions, or the whole answer task into one criterion. A match to any criterion admits the record. Retrieve partial evidence, then reason over the returned context yourself. Example: 'Do I go to the gym more often now?' -> ['Does the user mention going to the gym?']. Retrieve the individual observations first; compare their values and dates afterwards. For personalized advice, first retrieve the user's experiences with the topic, things they use, and stated likes or dislikes. Do not require the passage to explain how those facts solve the current problem. Use remembered constraints and resources in your answer. Distinguish the user's own statements from assistant suggestions and hypothetical examples while reading the returned context. For a linked fact, retrieve the known event or relationship before searching for the entity actually found. Do not assume an unknown answer, specialized meaning, or exact date in a criterion. For temporal questions, find the reference event before calculating a cutoff; use the question date only when the question refers to now. If a search is empty or yields only tangential evidence, use the remaining search budget to try a simpler topic criterion, dropping assumptions and optional qualifiers. An empty search alone does not prove the memory lacks the topic. Cite returned evidence, preserve uncertainty where a required fact is missing, and do not invent facts. Always supply evidence_questions for PerfectRecall quality mode; omitted criteria use generic query relevance.", 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'Natural language query.'}, 'evidence_questions': {'type': 'array', 'minItems': 1, 'maxItems': 3, 'items': {'type': 'string', 'minLength': 1, 'maxLength': 600}, 'description': "Caller-authored yes/no questions describing evidence to retrieve. Follow the tool's formulation instructions."}, 'limit': {'type': 'integer', 'description': 'Max results. Default 5.', 'default': 5}, 'temporal_weight': {'type': 'number', 'description': 'How much to boost recent memories (0.0 = ignore time, 0.2 = mild recency bias, 0.5 = strong recency bias). Default 0.0.', 'default': 0.0}, 'query_time': {'type': 'string', 'description': "ISO timestamp to treat as 'now' for temporal scoring. Omit to use the current time."}, 'temporal_halflife': {'type': 'number', 'description': 'Hours until temporal boost decays by half. Default 24. Lower = faster decay.', 'default': 24}, 'vec_weight': {'type': ['number', 'null'], 'description': 'Vector similarity weight in hybrid scoring. Omit (or pass null) to resolve config.yaml, then MNEMOSYNE_VEC_WEIGHT, then built-in default 0.5.'}, 'fts_weight': {'type': ['number', 'null'], 'description': 'Full-text search weight in hybrid scoring. Omit (or pass null) to resolve config.yaml, then MNEMOSYNE_FTS_WEIGHT, then built-in default 0.3.'}, 'importance_weight': {'type': ['number', 'null'], 'description': 'Importance score weight in hybrid scoring. Omit (or pass null) to resolve config.yaml, then MNEMOSYNE_IMPORTANCE_WEIGHT, then built-in default 0.2.'}, 'explain': {'type': 'boolean', 'description': 'If true, return a structured per-query recall explain trace. Default false.', 'default': False}}, 'required': ['query']}}
...
SHARED_RECALL_SCHEMA = {'name': 'mnemosyne_shared_recall', 'description': "Search only shared surface memory. Search PerfectRecall memory. Jev evaluates every eligible memory and returns original evidence records. Supply query and 1-3 evidence_questions. Each criterion must be a SHORT, self-contained yes/no question about ONE observable fact or topic; aim for 20 words or fewer. Use ordinary concrete words. Do not pack lists of attributes, eligibility conditions, or the whole answer task into one criterion. A match to any criterion admits the record. Retrieve partial evidence, then reason over the returned context yourself. Example: 'Do I go to the gym more often now?' -> ['Does the user mention going to the gym?']. Retrieve the individual observations first; compare their values and dates afterwards. For personalized advice, first retrieve the user's experiences with the topic, things they use, and stated likes or dislikes. Do not require the passage to explain how those facts solve the current problem. Use remembered constraints and resources in your answer. Distinguish the user's own statements from assistant suggestions and hypothetical examples while reading the returned context. For a linked fact, retrieve the known event or relationship before searching for the entity actually found. Do not assume an unknown answer, specialized meaning, or exact date in a criterion. For temporal questions, find the reference event before calculating a cutoff; use the question date only when the question refers to now. If a search is empty or yields only tangential evidence, use the remaining search budget to try a simpler topic criterion, dropping assumptions and optional qualifiers. An empty search alone does not prove the memory lacks the topic. Cite returned evidence, preserve uncertainty where a required fact is missing, and do not invent facts. Always supply evidence_questions for PerfectRecall quality mode; omitted criteria use generic query relevance.", 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string'}, 'evidence_questions': {'type': 'array', 'minItems': 1, 'maxItems': 3, 'items': {'type': 'string', 'minLength': 1, 'maxLength': 600}, 'description': "Caller-authored yes/no questions describing evidence to retrieve. Follow the tool's formulation instructions."}, 'limit': {'type': 'integer', 'default': 5}}, 'required': ['query']}}
...
class MnemosyneMemoryProvider(HermesPersonaPromptMixin, MemoryProvider):
    """PerfectRecall memory: SQLite persistence with Jev decisions."""
    ...
    def _should_filter(self, content: str) -> bool:
        ...
        if jev.enabled():
            ...
            allowed, _ = should_remember(content, ignore_patterns=self._ignore_patterns,
    ...
    def save_config(self, values: Dict[str, Any], hermes_home: str) -> None:
        ...
        try:
            ...
            memory_cfg = config.setdefault("memory", {}).setdefault("perfectrecall", {})
    ...
    def system_prompt_block(self) -> str:
        if self._beam:
            ...
            base = (
                ...
                "\n"
                "When a `## PerfectRecall Context` block is injected into the current turn, "
    ...
    def _prefetch_identity(self, existing_blocks: List[str], profile: "PrefetchProfile") -> str:
        ...
        lines: List[str] = ["## PerfectRecall Context"]
        seen: set = set()
        for r in rows:
    ...
    def _prefetch_model_slots(self, query: str, profile: "PrefetchProfile") -> str:
        ...
        content_limit = _prefetch_content_char_limit() or profile.content_char_limit
        lines = ["## PerfectRecall Model Context"]
    ...
    def _handle_recall(self, args: Dict[str, Any]) -> str:
        ...
        if "evidence_questions" in args:
            recall_kwargs["evidence_questions"] = args["evidence_questions"]
            ...
            if self._surface_beam is not None:
                try:
                    surface_results = self._surface_beam.recall(query, top_k=top_k,
                        **({"evidence_questions": args["evidence_questions"]} if "evidence_questions" in args else {}))
    ...
    def _handle_stats(self, args: Dict[str, Any]) -> str:
        ...
        return json.dumps({"provider": "perfectrecall", "session_id": self._session_id, "working": working, "episodic": episodic, "memoria": memoria})
        ...
        for pid in pending_ids:
            ...
            try:
                ...
                if (
                    ...
                    or record.get("provider") not in {"mnemosyne", "perfectrecall"}
                ):
                    failed.append({
                        "id": pid,
